...
SELECT
    ...
    CAST(((SUM(t1.Revenue) / t22.TotalRevenue) * 100) AS DECIMAL(10,2)) AS [2022 Rev Share (%)],
    SUM(t0.Revenue) AS [2015 Revenue (€m)],
    CAST(((SUM(t0.Revenue) / t15.TotalRevenue) * 100) AS DECIMAL(10,2)) AS [2015 Rev Share (%)],
    SUM(t1.Revenue) - SUM(t0.Revenue) AS [Growth (€m)],
    --FORMAT(ROUND(((SUM(t1.Revenue) - SUM(t0.Revenue)) / SUM(t0.Revenue)) * 100, 2), 'N', 'en-gb') AS [Growth (%)],
    CAST(((SUM(t1.Revenue) - SUM(t0.Revenue)) / SUM(t0.Revenue)) * 100 AS DECIMAL(10,2)) AS [Growth (%)],
    --POWER((SUM(t1.Revenue) / SUM(t0.Revenue)), (1.0 / ((t1.Season - t0.Season)))) - 1 AS [CAGR (%)],
    CAST((POWER((SUM(t1.Revenue) / SUM(t0.Revenue)), (1.0 / ((t1.Season - t0.Season)))) - 1) * 100 AS DECIMAL(10,2)) AS [CAGR (%)]
FROM MLGrowthBySeasonAndGrouping t1
LEFT OUTER JOIN MLGrowthBySeasonAndGrouping t0 
    --ON t1.Season = t0.Season + 1 --Per Year
    ON t1.Season = t0.Season + 7 --Per Period

...
SELECT
   SQRT(SUM((TotalRevenue - CombinedMeanRevenue) * (TotalRevenue - CombinedMeanRevenue)) / COUNT(TotalRevenue)) AS CombinedRevenueStandardDeviation
FROM PLRevenueByClub
CROSS JOIN (
   SELECT AVG(TotalRevenue) AS CombinedMeanRevenue
   FROM PLRevenueByClub
) AS MeanData;
